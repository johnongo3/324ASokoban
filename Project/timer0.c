...
static volatile uint8_t print_time; // Flag to control printing
volatile uint32_t seconds_elapsed = 0;
volatile uint32_t milliseconds_elapsed = 0;

// for tone playing
volatile uint8_t tone_playing;
volatile uint16_t tone_duration_ms;
...
void start_printing_time(void) {
	print_time = 1; // Enable printing
}

void stop_printing_time(void) {
	print_time = 0; // Disable printing
}
...
ISR(TIMER0_COMPA_vect)
{
	...
	if (print_time) {
		...
		if (milliseconds_elapsed >= 1000) {
			...
			move_terminal_cursor(12, 25);
			printf("\rTime elapsed: %lu seconds", seconds_elapsed);
		}
	}
	
	if (tone_playing == 1 && tone_duration_ms > 0) {
		tone_duration_ms--;
